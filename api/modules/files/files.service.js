...
const buildBreadcrumbs = async (parent) => {
    ...
    const ancestors = await findAncestorNodes(ancestorIds);
...
const updateDescendantAncestors = async (folderId, previousAncestors, newAncestors) => {
    const descendants = await findDescendantNodes(folderId);
    ...
    await Promise.all(
        descendants.map((node) => {
            ...
            const idx = current.indexOf(folderIdString);
            let updatedAncestors;

            if (idx === -1) {
                updatedAncestors = current;
            } else {
                const remainder = current.slice(idx + 1);
            ...
            return updateNodeById(node._id, {
...
export const buildFolderTree = (folders) => {
    ...
    folders.forEach((folder) => {
        const parentId = folder.parent ? folder.parent.toString() : 'root';
    ...
    const attachChildren = (parentId = 'root') =>
        (byParent.get(parentId) || []).map((node) => ({
            ...node,
            children: attachChildren(node.id.toString()),
        }));
...
export const listDirectory = async ({ parentId = null }) => {
    ...
    if (parentId) {
        ...
        parent = await findNodeById(parentId);
        if (!parent) {
    ...
    const items = await findDirectoryItems(filter);
...
export const createFolder = async ({ name, parentId = null }) => {
    ...
    const duplicate = await findNodeByNameInParent({
...
export const handleFileUpload = async ({ parentId = null, uploadParent = null, file }) => {
    ...
    const duplicate = await findNodeByNameInParent({
        parentId: uploadParent ? uploadParent._id : null,
        name: displayName,
    });
    ...
    const fileNode = createNodeRecord({
        name: displayName,
        type: 'file',
    ...
    const finalRelativePath = joinStorageSegments(
        'files',
        extension
            ? `${fileNode._id.toString()}.${extension}`
            : fileNode._id.toString()
    );
...
export const updateNode = async ({ id, name, newParentId }) => {
    ...
    if (typeof newParentId !== 'undefined') {
        if (newParentId) {
            ...
            parent = await findNodeById(newParentId);
            if (!parent || parent.type !== 'folder') {
                throw errorHandler(400, 'Target folder not found');
            }
        } else {
        ...
        parent = await findNodeById(node.parent);
    }
    ...
    const effectiveParent = parentChanged
        ? parent
        : parent ?? (node.parent ? await findNodeById(node.parent) : null);
    ...
    const oldAncestors = node.ancestors.map((ancestorId) => ancestorId.toString());
    ...
    if (Object.keys(updates).length > 0) {
        await updateNodeById(node._id, updates);
    }
    ...
    const fresh = await findNodeById(node._id);
...
export const deleteNode = async ({ id }) => {
    ...
    const related = await findNodesForDeletion(node._id);
    ...
    await deleteNodesByIds(related.map((item) => item._id));
...
export const downloadFile = async ({ id }) => {
    ...
    try {
        ...
        throw errorHandler(500, 'Unable to access the requested file.');
    }
